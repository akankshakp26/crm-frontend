...
const Dashboard = () => {
  ...
  const totalRevenue = leads.reduce(
    (sum, l) =>
      sum + (l.stage === 'Closed Won' ? (Number(l.value) || 0) : 0),
    0
  );
  ...
  return (
    <div className="p-8 animate-in fade-in duration-700">
      <h1 className="text-4xl font-black text-slate-900 mb-10 tracking-tight text-left">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
        ...
        <div className="bg-slate-900 p-10 rounded-[2.5rem] text-white shadow-xl">
          <p className="text-slate-400 font-bold uppercase text-xs tracking-widest text-left">
            Active Pipeline Leads
          </p>
          <h3 className="text-5xl font-black mt-2 text-left">
            {activeLeads}
          </h3>
        </div>
      </div>

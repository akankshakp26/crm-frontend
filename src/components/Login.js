...
const LoginPage = ({ onLogin }) => {
  ...
  const [loginType, setLoginType] = useState("sales"); // "sales" acts as the Client role
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If user logs in as 'sales' (Client), ensure they aren't an admin
      if (loginType === "sales" && loggedUser.role === "admin") {
        setError("Access denied: Client login only");
        return;
      }
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: '#f1f5f9' }}>
      <div style={{ backgroundColor: '#fff', borderRadius: '32px', overflow: 'hidden', width: '850px', minHeight: '520px', position: 'relative', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.2)' }}>
        ...
        {/* FORM SIDE */}
        <div style={{ position: 'absolute', top: 0, height: '100%', width: '50%', left: '0', transition: 'all 0.6s cubic-bezier(0.4, 0, 0.2, 1)', zIndex: 20 }}>
        ...
        {/* STATIC PANEL (Replaces Sliding Panel) */}

...
const Layout = ({ children, onLogout, user, searchTerm, setSearchTerm }) => {
  ...
  // ✅ Fetch + realtime notifications (won't crash on 404/401)
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        const res = await axiosInstance.get(`/notifications/${storedUser.id}`);
  ...
  const clearAll = async () => {
    try {
      ...
      await axiosInstance.put(`/notifications/read-all/${storedUser.id}`);
      setNotifications([]);
    } catch (error) {
      ...
      <aside className="group w-20 hover:w-64 bg-slate-900/95 backdrop-blur-xl text-white flex flex-col transition-all duration-300 ease-in-out z-30 border-r border-white/10 shadow-2xl relative no-scrollbar">
        <div className="p-5 mb-10 flex-shrink-0">
          <div className="flex items-center gap-4 overflow-hidden">
            <div className="h-10 w-10 min-w-[40px] rounded-xl bg-blue-600 flex items-center justify-center font-bold shadow-lg ring-1 ring-white/30 text-white flex-shrink-0">
              V
            </div>
            <span className="text-xl font-bold opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">
              Valise CRM
            </span>
          </div>
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                key={item.name}
                to={item.path}
                className={`flex items-center gap-4 p-3 rounded-2xl transition-all duration-200 group/item border ${
                  isActive
              ...
              >
                ...
                />
                <span className="font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap">
                  {item.name}
                </span>
              </Link>
            );
        ...
        <div className="p-4 border-t border-white/5 bg-black/20">
          <button
            onClick={onLogout}
            className="flex items-center gap-4 p-3 w-full rounded-xl text-slate-400 hover:bg-red-500/20 hover:text-red-400 transition-all duration-200"
          >
        ...
        <header className="h-20 bg-white/70 backdrop-blur-lg border-b border-slate-200/60 flex items-center px-8 justify-between sticky top-0 z-20">
          <div className="relative w-96">
            ...
            <input
              ...
              className="w-full bg-slate-100/80 rounded-2xl py-2.5 pl-11 pr-4 focus:ring-2 focus:ring-blue-500 outline-none font-medium transition-all"
              value={searchTerm}
              ...
              {showProfileMenu && (
                <div className="absolute right-0 top-[85%] w-48 bg-white rounded-3xl border border-slate-200 shadow-2xl p-2 z-50">
                  <button
                    ...
                    className="w-full flex items-center gap-3 p-3 text-slate-600 hover:bg-slate-50 rounded-2xl font-bold text-xs text-left"
                  >
                  ...
                  <button
                    onClick={onLogout}
                    className="w-full flex items-center gap-3 p-3 text-red-500 hover:bg-red-50 rounded-2xl font-bold text-xs text-left"
                  >
